...
.factory('calListeners', function(){
  ...
  return {
    ...
    drop: function (targetDay, originDay) {
      if (listeners.drop) {
...
.directive('calendar', function(Calendar, $parse, calListeners){
  // Runs during compile
  return {
    scope: {},
    restrict: 'A',
    ...
    link: function($scope, iElm, attrs, controller) {
      ...
      var populate = $parse(attrs.calPopulate);
    ...
    $document.bind('mouseup', function (ev) {
      ...
      if (dropArea.length > 0) {
        ...
        if (targetList !== dragOrigin) {
          ...
          calListeners.drop(targetScope.day, originScope.day);
